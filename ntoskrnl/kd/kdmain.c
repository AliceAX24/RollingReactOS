...
KdDebuggerInitialize0(
    In_opt PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    PSTR CommandLine, Port = NULL, BaudRate = NULL;
    ULONG i;
    BOOLEAN Success = FALSE;
    BOOLEAN NoDebugMode = FALSE;
    ...
    if (LoaderBlock)
    {
        ...
        if (CommandLine)
        {
            /* Upcase it for easier parsing */
            _strupr(CommandLine);

            /* Check for explicit NODEBUG flag first for performance */
            if (strstr(CommandLine, "NODEBUG"))
            {
                NoDebugMode = TRUE;
                KdpDebugMode.Value = 0; // Ensure all debug modes are off
            }
            else
            {
    ...
    /* Only process debug ports if NODEBUG was not specified */
    if (!NoDebugMode)
    {
    ...
    /* Call the providers at Phase 0 only if debug is enabled */
    if (!NoDebugMode)
    {
    ...
    /* Return success if at least one of the providers succeeded or if debug is disabled */
    return (NoDebugMode ? STATUS_SUCCESS : (Success ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL));
}
